...
Now, If we have higher dimensions, by higher dimensions I simply mean we still have a **scalar function** but **x** now is a vector. So in that case, $\large \vec{x}$ vector could be something like let us say X 1, X 2, X 3, or the figure that I will show shortly could be X factor is X 1, X 2 which means X belongs to R 2, this is the case where X belongs to R 3 okay.
