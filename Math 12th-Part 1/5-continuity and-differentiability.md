...
In other words, let say we have a function **_y=f(x)_** if we differentiate it we get the **1<sup>st</sup> order derivative** which is  $\large\frac{dy}{dx}=f^\prime(x)$  if we further differentiate it we get the **2<sup>nd</sup> order derivative** $\large\frac{d}{dx}\Big \(\frac{dy}{dx}\Big\)=\frac{d^2{dx^2}=f\Pime(x)$ 
* Let **_y = f (x)_**.
